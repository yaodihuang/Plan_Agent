...
func buildPlanPrompt(query, projectName, parentBranchID, reviewMapContent, codeAnalysisContext string) string {
	...
	// Inject review map content at the very beginning if available
	if strings.TrimSpace(reviewMapContent) != "" {
		sb.WriteString("=== EXISTING REVIEW MAP (USE THIS AS CONTEXT) ===\n")
	...
	// Conditionally adjust review workflow based on existing review map
	if strings.TrimSpace(reviewMapContent) != "" {
		sb.WriteString("Review task workflow requirements (EXISTING REVIEW MAP DETECTED):\n")
		sb.WriteString("An existing review map has been provided above. When the query requests review:\n")
		sb.WriteString("1. SKIP generating a new review map - use the existing one as the authoritative source\n")
		sb.WriteString("2. If the existing map appears outdated or incomplete for the current task:\n")
		sb.WriteString("   - Include a brief 'validate/update review map' step ONLY if necessary\n")
		sb.WriteString("   - This step should UPDATE the existing map, not recreate it from scratch\n")
		sb.WriteString("3. Proceed directly to review execution based on the existing map:\n")
		sb.WriteString("   - Define a systematic review strategy referencing sections from the existing map\n")
		sb.WriteString("   - Decompose review work into subtasks aligned with the map's structure\n")
		...
		sb.WriteString("4. Task dispatch requirements:\n")
	...
	sb.WriteString("5. read_file - Read a local file from the workspace directory\n")
	sb.WriteString("   Args: {path: string}\n")
	sb.WriteString("   - path: File path relative to workspace directory, or absolute path within workspace\n")
	sb.WriteString("   Use for: Loading context files (e.g., review-map.md) before generating plans\n")
	sb.WriteString("   IMPORTANT: If context files like review-map.md exist, read them FIRST to inform your planning\n\n")
